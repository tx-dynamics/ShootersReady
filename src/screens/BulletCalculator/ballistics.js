...
Ballistics.prototype.setVariables = function (variables) {
...
Ballistics.prototype.solveAll = function () {
  var deltaTime = 0;
  ...
  for (var time = 0; ; time = time + deltaTime) {
    let velocityX1 = velocityX;
    let velocityY1 = velocityY;
...
Ballistics.prototype.getHeadWind = function (windAngle, windSpeed) {
  return Math.cos(this.degToRad(windAngle)) * windSpeed;
};

Ballistics.prototype.getCrossWind = function (windAngle, windSpeed) {
  return Math.sin(this.degToRad(windAngle)) * windSpeed;
};

Ballistics.prototype.getWindageDeflection = function (
  windSpeed,
  muzzleVelocity,
  range,
  time,
) {
  let windVelocity = windSpeed * 17.6; // Convert to inches per second.
  return windVelocity * (time - range / muzzleVelocity);
};

Ballistics.prototype.degToRad = function (degrees) {
  return (degrees * Math.PI) / 180.0;
};

Ballistics.prototype.radToDeg = function (radians) {
  return (radians * 180.0) / Math.PI;
};

Ballistics.prototype.minuteOfAngleToRad = function (minuteOfAngle) {
  return ((minuteOfAngle / 60.0) * Math.PI) / 180.0;
};

Ballistics.prototype.radToMinuteOfAngle = function (radians) {
  return (radians * 60.0 * 180.0) / Math.PI;
};

Ballistics.prototype.getZeroAngle = function () {
  var time = 0;
  ...
  for (angle = 0; ; angle = angle + deltaAngle) {
    let velocityY = this.muzzleVelocity * Math.sin(angle);
    let velocityX = this.muzzleVelocity * Math.cos(angle);
    let gravityVectorX = this.gravity * Math.sin(angle);
    let gravityVectorY = this.gravity * Math.cos(angle);
    ...
    for (
      ...
      let dragVelocity = this.retard(velocity);
...
Ballistics.prototype.retard = function (velocity) {
  let A = -1;
  let M = -1;

  let dragFunction = this.DRAG_FUNCTIONS[this.dragFunction];
  if (typeof dragFunction !== 'undefined') {
    let thresholdsLength = dragFunction.thresholds.length;
  ...
  if (A != -1 && M != -1 && velocity > 0 && velocity < 10000) {
    let retardedVelocity = (A * Math.pow(velocity, M)) / this.dragCoefficient;
...
Ballistics.prototype.getRange = function (yardage) {
...
Ballistics.prototype.getPath = function (yardage) {
  if (yardage < this.maximumValidRange) {
...
Ballistics.prototype.getMinuteOfAngle = function (yardage) {
  if (yardage < this.maximumValidRange) {
...
Ballistics.prototype.getTime = function (yardage) {
  if (yardage < this.maximumValidRange) {
...
Ballistics.prototype.getWindage = function (yardage) {
  if (yardage < this.maximumValidRange) {
...
Ballistics.prototype.getWindageMOA = function (yardage) {
  if (yardage < this.maximumValidRange) {
...
Ballistics.prototype.getVelocity = function (yardage) {
  if (yardage < this.maximumValidRange) {
...
Ballistics.prototype.getVelocityX = function (yardage) {
  if (yardage < this.maximumValidRange) {
...
Ballistics.prototype.getVelocityY = function (yardage) {
  if (yardage < this.maximumValidRange) {
